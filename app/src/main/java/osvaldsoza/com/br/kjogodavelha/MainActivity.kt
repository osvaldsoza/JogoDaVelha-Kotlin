package osvaldsoza.com.br.kjogodavelha
...
class MainActivity : AppCompatActivity() {
    ...
    var playerOne = arrayListOf<Int>()
    var playerTwo = arrayListOf<Int>()
    var currentPlayer = 1
    ...
    fun play(position: Int, btnSelecionado: Button) {
        if (currentPlayer == 1) {
            btnSelecionado.text = "X"
            btnSelecionado.setBackgroundResource(R.color.colorPlayer1)
            playerOne.add(position)
            currentPlayer = 2
        } else {
            btnSelecionado.text = "O"
            btnSelecionado.setBackgroundResource(R.color.colorPlayer2)
    ...
    fun btnPositon(view: View) = when (view.id) {
